    ...
    try {
      ...
      if (data?.errors) {
        ...
        data.errors.forEach((e: string) => {
          const field = e.toLowerCase().includes('email') ? 'email' : e.toLowerCase().includes('password') ? 'password' : 'general';
          map[field] = e;
        });
        setErrors(map);
      } else {
    ...
    <GuestLayout>
      ...
      <div className="mt-4">
        ...
        <a
          ...
          className="mt-3 w-full flex justify-center items-center gap-2 rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors"
        >
